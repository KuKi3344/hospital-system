	...
	<div>
		...
		<el-table v-loading="loading" :data="dataList" height="480" border style="width:100%; border-radius:10px;font-size: 13px;"
			...
			<el-table-column label="操作" width="210" height="40">
				...
				<template slot-scope="scope">
					<el-button size="mini" type="primary"  @click="confirmlist(scope.row.id)">结束问诊</el-button>
				</template>
		...
		methods:{
			...
			confirmlist(id){
					this.$axios.get('/api/sub/select/one/'+id).then(resp=>{
						...
						this.updateList.enquiryStatus = "问诊结束";
						this.getdetailTime();
						this.updateList.handleEndDate = this.timeid;
